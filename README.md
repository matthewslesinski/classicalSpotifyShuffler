...
- Rate Limit handling: Spotify's API is guarded with [rate limits](https://developer.spotify.com/documentation/web-api/guides/rate-limits/), tracked per app, that limit the number of requests that get a meaningful response within a given span of time. I hope to implement a throttling mechanism that aims to allow sending as many concurrent requests without waiting while also minimizing the frequency of hitting the rate limit.
